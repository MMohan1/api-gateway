...
class requestHandlerTest(TestCase):
    def setUp(self):
        ...
        self.rgate_config = {
            ...
            "backends": [
                {
                    "name": "payment",
                    "match_labels": ["app_name=payment", "env=production"],
                },
                {
                    "name": "orders",
                    "match_labels": ["app_name=orders", "env=production"],
                },
            ],
        }
    ...
    def test_handle_request(self):
        ...
        with self.subTest("When backend container exists and running"):
            ...
            mockObject.get_container_ip_and_port = mock.Mock(
                return_value=ip_and_port_return_value
            )
